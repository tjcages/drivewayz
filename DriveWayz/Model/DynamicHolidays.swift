//
//  DynamicHolidays.swift
//  DriveWayz
//
//  Created by Tyler Jordan Cagle on 2/9/19.
//  Copyright © 2019 COAD. All rights reserved.
//

import Foundation

extension dynamicPricing {
    
    static func getHolidayPricing(averagePrice: CGFloat, completion: @escaping(CGFloat) -> ()) {
        let date = Date()
        ...
        if let holiday = Date.martinLKDay(year: currentYear) {
            let holidayString = formatter.string(from: holiday)
            if holidayString == currentDate {
                let holidayPrice = averagePrice + averagePrice * 0.2
                completion(holidayPrice)
        ...
        if let holiday = Date.columbusDay(year: currentYear) {
            let holidayString = formatter.string(from: holiday)
            if holidayString == currentDate {
                let holidayPrice = averagePrice + averagePrice * 0.2
                completion(holidayPrice)
        ...
        if let holiday = Date.veteransDay(year: currentYear) {
            let holidayString = formatter.string(from: holiday)
            if holidayString == currentDate {
                let holidayPrice = averagePrice + averagePrice * 0.2
                completion(holidayPrice)
...
extension Date {
    var isUSHoliday: Bool {
        ...
        case (1, 0, 2, 3): return true                      // Martin Luther King - 3rd Mon in Jan
        case (2, 14, _, _): return true                     // Valentine day - 14th in Feb
        case (2, 0, 2, 3): return true                      // Washington's Birthday - 3rd Mon in Feb
        ...
        case (5, 0, 1, 2): return true                      // Mothers day - 2nd Sun in May
        case (5, memorialDay, _, _): return true            // Memorial Day
        case (5, 0, 1, 3): return true                      // Fathers day - 3rd Sun in May
