    ...
    switch (_directionsService) {
        ...
        default:
        {
            NSDictionary *userInfo = @{
                                       NSLocalizedDescriptionKey: NSLocalizedString(@"Operation was unsuccessful 1.", nil),
...
- (void)calculateDirectionsGoogleMaps {
    ...
    if(_transportType == MKDirectionsTransportTypeWalking)
        mode = @"walking";
    NSString *key = @"AIzaSyDbzBQIrY1DDfKxuA1_Vw_NMvOFoJxAH8U"; //REMOVE
    
    NSString *requestURL = [NSString stringWithFormat:
//                            @"http://maps.googleapis.com/maps/api/directions/json?origin=%f,%f&destination=%f,%f&sensor=true&mode=%@&key=%@",
                                @"%@?origin=%f,%f&destination=%f,%f&mode=%@&key=%@&sensor=true",
    ...
    NSURL *directionsURL = [NSURL URLWithString:requestURL];
    
//    NSURL *url = [NSURL URLWithString:[requestURL stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
//    NSURL *url = [NSURL URLWithString:[requestURL stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLHostAllowedCharacterSet]]];
...
//        NSError *error = nil;
        ...
        if(error) {
            if([self->delegate respondsToSelector:@selector(navigationKitError:)])
                [self->delegate navigationKitError:error];
            return;
        }
        ...
        if(!routes) {
            if([self->delegate respondsToSelector:@selector(navigationKitError:)]) {
                NSDictionary *userInfo = @{
                                           NSLocalizedDescriptionKey: NSLocalizedString(@"Operation was unsuccessful 2.", nil),
                                           NSLocalizedFailureReasonErrorKey: NSLocalizedString(@"Could not find any routes for specified locations", nil)
                                           };
                [self->delegate navigationKitError:[NSError errorWithDomain:NavigationKitErrorDomain code:-2 userInfo:userInfo]];
            }
            return;
        }
        ...
        if(!route) {
            if([self->delegate respondsToSelector:@selector(navigationKitError:)]) {
                NSDictionary *userInfo = @{
                                           NSLocalizedDescriptionKey: NSLocalizedString(@"Operation was unsuccessful 3.", nil),
                                           NSLocalizedFailureReasonErrorKey: NSLocalizedString(@"Could not find any routes for specified locations", nil)
                                           };
                [self->delegate navigationKitError:[NSError errorWithDomain:NavigationKitErrorDomain code:-2 userInfo:userInfo]];
            }
        ...
        self->_route = [[NKRoute alloc] initWithGoogleMapsRoute:route];
        
        if([self->delegate respondsToSelector:@selector(navigationKitCalculatedRoute:)])
            [self->delegate navigationKitCalculatedRoute:self->_route];
    }] resume];
}
...
- (void)calculateDirectionsAppleMaps {
    ...
    [directions calculateDirectionsWithCompletionHandler:^(MKDirectionsResponse *response, NSError *error) {
        ...
        self->_route = [[NKRoute alloc] initWithMKRoute:response.routes.firstObject];
...
- (GMSCameraPosition *)defaultCamera:(CLLocation *)location {
    return [GMSCameraPosition cameraWithTarget:[location coordinate] zoom:_cameraAltitude == -1 ? 1500 : _cameraAltitude];
//            cameraLookingAtCenterCoordinate:[location coordinate] fromEyeCoordinate:[location coordinate] eyeAltitude:_cameraAltitude == -1 ? 500 : _cameraAltitude];
}

// Calculate the camera based on the users settings
- (GMSCameraPosition *)cameraForStep:(NKRouteStep *)step location:(CLLocation *)location {
    ...
    GMSCameraPosition *newCamera = [GMSCameraPosition cameraWithTarget:coordinateWithOffset zoom:_cameraAltitude == -1 ? 500 : _cameraAltitude bearing:heading viewingAngle:60];
