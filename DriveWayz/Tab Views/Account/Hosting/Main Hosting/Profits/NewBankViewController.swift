...
class NewBankViewController: UIViewController {
    ...
    var delegate: handleBankTransfers?
        ...
        ref.observeSingleEvent(of: .value) { (snapshot) in
            if let dictionary = snapshot.value as? [String: Any] {
                ...
                if let phone = dictionary["phone"] as? String {
                    self.namesController.phoneTextField.text = phone
                }
...
extension NewBankViewController {
    ...
    func registerAccount() {
        ...
        if let firstName = self.namesController.nameTextField.text, let lastName = self.namesController.lastTextField.text, let phoneNumber = self.namesController.phoneTextField.text, let emailAddress = self.namesController.emailTextField.text, let addressLine1 = self.locationController.address1TextField.text, let cityAddress = self.locationController.cityTextField.text, let stateAddress = self.locationController.stateLabel.text, let zipCode = self.locationController.zipTextField.text, let ssnCode = self.ssnController.ssnTextField.text?.replacingOccurrences(of: " ", with: ""), let routingNumber = self.routingController.routingTextField.text, let accountNumber = self.routingController.accountTextField.text, let birthDay = self.dobController.dayTextField.text, let birthMonth = self.dobController.monthTextField.text, let birthYear = self.dobController.yearTextField.text {
            MyAPIClient.sharedClient.createAccountKey(routingNumber: routingNumber, accountNumber: accountNumber, addressLine1: addressLine1, addressCity: cityAddress, addressState: stateAddress, addressPostalCode: zipCode, firstName: firstName, lastName: lastName, ssnLast4: ssnCode, phoneNumber: phoneNumber, email: emailAddress, birthDay: birthDay, birthMonth: birthMonth, birthYear: birthYear) { (success) in
                ...
                if let dictionary = snapshot.value as? [String:AnyObject] {
                    if let account = dictionary["accountID"] as? String {
                        if let userFunds = dictionary["userFunds"] as? Double {
                            self.delegate?.dismissTransferController()
                            delayWithSeconds(animationIn * 2 + 0.1, completion: {
                                self.delegate?.bringTransferCountroller(accountID: account, userFunds: userFunds)
                            })
                        } else {
                            self.delegate?.dismissTransferController()
                        }
