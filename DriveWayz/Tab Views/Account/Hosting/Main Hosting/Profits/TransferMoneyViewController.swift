...
class TransferMoneyViewController: UIViewController {
    ...
    func setupAccount(accountID: String, userFunds: Double) {
                ...
                MyAPIClient.sharedClient.transferToBank(account: accountID, funds: userFunds) { (success) in
                    if success == true {
                        if let currentUser = Auth.auth().currentUser?.uid {
                            ...
                            payRef.updateChildValues(["timestamp": Date().timeIntervalSince1970, "transferAmount": userFunds, "accountID": accountID])
