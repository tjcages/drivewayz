...
class ExpandedBookingsViewController: UIViewController {
    ...
    func setData(booking: Bookings, parking: ParkingSpots) {
        if let userName = booking.userName, let userDuration = booking.userDuration, let userProfileURL = booking.userProfileURL, let userRating = booking.userRating, let fromDate = booking.fromDate {
            ...
            if userProfileURL == "" {
                ...
                self.profileImageView.loadImageUsingCacheWithUrlString(userProfileURL) { (bool) in
                    if !bool {
                        self.profileImageView.image = UIImage(named: "background4")
                    }
                }
    ...
    func setupVehicle() {
        ...
        userVehicle.leftAnchor.constraint(equalTo: self.view.leftAnchor, constant: 20).isActive = true
        ...
        userLicense.leftAnchor.constraint(equalTo: self.view.leftAnchor, constant: 20).isActive = true
