...
class BookingHelpView: UIViewController {

    var delegate: BookingHelpDelegate?
            ...
            if let options = self.allOptions {
                if let questions = options.questionTopics {
                    ...
                    for question in questions {
                        ...
                        if title == "Issues With Motorists" {
                        ...
                        } else if title == "Driver Overstay Options" {
                            self.options[title] = question.questions
                            if let count = question.questions?.count {
                                optionsHeightAnchor.constant += CGFloat(63 + count * 60)
                            }
    ...
    var bookingContainer: BookingsContainerView = {
    ...
    override func viewDidAppear(_ animated: Bool) {
        if gradientController.mainLabel.text != "Booking Help" {
            gradientController.setMainLabel(text: "Booking Help")
        }
    }
    ...
    func setupViews() {
        ...
        scrollView.addSubview(bookingContainer)
        
        mainLabel.leftAnchor.constraint(equalTo: view.leftAnchor, constant: 20).isActive = true
        mainLabel.topAnchor.constraint(equalTo: scrollView.topAnchor, constant: 32).isActive = true
        mainLabel.sizeToFit()
        
        bookingContainer.anchor(top: mainLabel.bottomAnchor, left: view.leftAnchor, bottom: nil, right: view.rightAnchor, paddingTop: 8, paddingLeft: 16, paddingBottom: 0, paddingRight: 16, width: 0, height: 128)
        
        scrollView.addSubview(optionsTableView)
        optionsTableView.anchor(top: bookingContainer.bottomAnchor, left: view.leftAnchor, bottom: nil, right: view.rightAnchor, paddingTop: 0, paddingLeft: 0, paddingBottom: 0, paddingRight: 0, width: 0, height: 0)
...
extension BookingHelpView: UITableViewDelegate, UITableViewDataSource {
...
extension BookingHelpView: UIScrollViewDelegate {
