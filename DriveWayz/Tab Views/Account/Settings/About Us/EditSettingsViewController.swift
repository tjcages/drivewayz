...
class EditSettingsViewController: UIViewController {
    ...
    @objc func updateButtonPressed() {
        ...
        if let title = detailLabel.text, let message = detailView.text, let userID = Auth.auth().currentUser?.uid {
            ...
            if title == "Email" {
                let values = ["email": message]
                ref.updateChildValues(values) { (err, ref) in
                    self.delegate?.changeEmail(text: message)
                }
            } else if title == "Phone" {
                let values = ["phone": message]
                ref.updateChildValues(values) { (err, ref) in
                    self.delegate?.changePhone(text: message)
                }
            } else if title == "First name" {
                if var name = currentUser?.name {
                    let split = name.split(separator: " ")
                    if let first = split.first {
                        name = name.replacingOccurrences(of: first, with: message)
                        let values = ["name": name]
                        ref.updateChildValues(values) { (err, ref) in
                            self.delegate?.changeName(text: name)
                        }
                    }
                } else {
                    let values = ["name": message]
                    ref.updateChildValues(values) { (err, ref) in
                ...
                if var name = currentUser?.name {
                    ...
                    if let last = split.last {
                        name = name.replacingOccurrences(of: last, with: message)
                        let values = ["name": name]
                        ref.updateChildValues(values) { (err, ref) in
                            self.delegate?.changeName(text: name)
                        }
